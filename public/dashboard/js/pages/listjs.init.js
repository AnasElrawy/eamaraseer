var customerList, checkAll = document.getElementById("checkAll"),
    perPage = (checkAll && checkAll.addEventListener("click", function() {
        var e = document.querySelectorAll('.form-check-all input[type="checkbox"]');
        Array.from(e).forEach(function(e) {
            e.checked = checkAll.checked, e.checked ? e.closest("tr").classList.add("table-active") : e.closest("tr").classList.remove("table-active")
        })
    }), 8),
    editlist = !1,
    options = {
...
}, xhttp.open("GET", "assets/json/table-customer-list.json"), xhttp.send();
...
var table = document.getElementById("customerTable"),
    ...
    statusVal = (Array.prototype.slice.call(forms).forEach(function(a) {
        a.addEventListener("submit", function(e) {
            var t;
            a.checkValidity() ? (e.preventDefault(), "" === customerNameField.value || "" === emailField.value || "" === dateField.value || "" === phoneField.value || editlist ? "" !== customerNameField.value && "" !== emailField.value && "" !== dateField.value && "" !== phoneField.value && editlist && (t = customerList.get({
                id: idField.value
                ...
                order: "desc"
            }), document.getElementById("close-modal").click(), refreshCallbacks(), clearFields(), filterContact("All"), count++, Swal.fire({
                position: "center",
                icon: "success",
