var icons = {
    ...
    "ph-arrow-circle-down-left-light": 61e3,
...
function loadIconList(h) {
    let l = "";
    for (const p of h) l += `<div class="col-xl-3 col-lg-4 col-sm-6"> <i class="${p}"></i> ${p} </div>`;
    document.getElementById("iconList").innerHTML = l
}
loadIconList(keys), document.getElementById("icon-select").addEventListener("change", function(h) {
    const l = h.target.value;
    loadIconList(l ? keys.filter(function(h) {
        return h.split("-").slice(-1).pop() === l
    }) : keys)
});